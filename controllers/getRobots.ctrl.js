...
methods.getRobots = async (req, res, next) => {
    try {
        let appSettings = appConfig.appSettings;
        ...
        if (appSettings.CommunityEnvironment == "Community") {
            ...
            items = await oapi.GetRobots(CommunityUrl + "/odata/Robots", EnvironmentName, sessionid, tenantName);
        }
        else {
            let oapi = new Orch_APIClass();
            items = await oapi.GetRobots(BaseUrl + "/odata/Robots", EnvironmentName, sessionid);
        }
