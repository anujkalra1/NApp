...
class Orch_APIClass {
    ...
    async Authenticate(AuthenticationURL, Tenant, Usernameoruseremail, Password){
        return new Promise(function(resolve, reject){ 
            try
            {
                let loginModel = {
                    "tenancyName": Tenant,
                    "usernameOrEmailAddress": Usernameoruseremail,
                    "password": Password
                }
                console.log("authenticate : "+AuthenticationURL)
                request({
    ...
    async GetRobots(AuthenticationURL, EnvironmentName, sessionid) {
    ...
    async GetEnvironments(AuthenticationURL, sessionid) {
        var userdetail = await SessionHelper.Get(sessionid);
        userdetail = JSON.parse(userdetail);
        let thisLocal = this;
        let isTrue = true;
    ...
    async CreateUser(AuthenticationURL, AuthToken, name, surname, username, password, roleList, tenantName) {
        return new Promise(async(resolve, reject) => {
            try {
                let objUser = {
                    "Name": name,
                ...
                request({
                    ...
                    if(response && response.statusCode == 201){
                        ...
                        console.log("Error while executing create user api"); 
                        console.log(error);
                        reject(error);
                    }			
                }); 
    ...
    async UpdateUser(AuthenticationURL, AuthToken, name, username, surname, password, roleList) {
        let thisLocal = this;
        return new Promise(async(resolve, reject) => {
            try {
                let userId = await thisLocal.GetUserId(AuthenticationURL, AuthToken, username);
                console.log("userId : "+userId)
                var userInfor = await thisLocal.GetUserById(AuthenticationURL, AuthToken, userId);
                console.log("userInfor before: ")
                console.log(userInfor)
                userInfor.Name = name;
                userInfor.Surname = surname;
                userInfor.RolesList = roleList;
                userInfor.Password = password;
                console.log("userInfor after: ")
                console.log(userInfor)
                request({
                    rejectUnauthorized: false,
                    url:AuthenticationURL + "(" + (userId) + ")",
                    method: 'PUT',
                    headers:{
                        "cache-control":"no-cache",
                        "Content-Type":"application/json",
                    ...
                    if(response && response.statusCode == 200){
                        ...
                        console.log("Error while executing update user api"); 
                        console.log(error);
                        reject(error);
                    }			
                }); 
    ...
    async GetUserId(AuthenticationURL, AuthToken, userName) {
        let userId = '';
        return new Promise(async(resolve, reject) => {
            try {
                request({
                    ...
                    if(response && response.statusCode == 200){
                        ...
                        console.log("Error while executing get userid api"); 
                        console.log(error);
                        reject(error);
                    }			
                }); 
    ...
    async GetUserById(AuthenticationURL, AuthToken, userId) {
        let user = {};
        return new Promise(async(resolve, reject) => {
            try {
                request({
                    ...
                    if(response && response.statusCode == 200){
                        ...
                        console.log("Error while executing get user by id api"); 
                        console.log(error);
                        reject(error);
                    }			
                }); 
    ...
    async UpdatePassword(AuthenticationURL, AuthToken, CurrentPassword, OldPassword, username, userEndpoint) {
        let thisLocal = this;
        return new Promise(async(resolve, reject) => {
            try {
                let userId = await thisLocal.GetUserId(userEndpoint, AuthToken, username);
                let objUser = {
    ...
    async DeleteUser(AuthenticationURL, AuthToken, username) {
        let thisLocal = this;
        return new Promise(async(resolve, reject) => {
            try {
                let userId = await thisLocal.GetUserId(AuthenticationURL, AuthToken, username);
                request({
                    ...
                    if(response && response.statusCode == 204){
                        ...
                        resolve("Inactive username provided. Please verify");
                    }			
                }); 
            } catch (e) {
                reject(e);
    ...
    async GetReleaseKey(AuthenticationURL, sessionid, ProcessName, Environmentname)
    {
        let thisLocal = this;
        let isTrue = true;
        let status = 0;
    ...
    async GetRobotID(AuthenticationURL, sessionid, RobotName)
    {
    ...
    async startJob(AuthenticationURL, AuthToken, ProcessName, RoboID, ReleaseKey, BusinessVarientPath, 
        UserSessionId, EYPAFilePath, ReportFilePath)
    {
        console.log('startJob-----');
        let userdetail = await SessionHelper.Get(UserSessionId);
        userdetail = JSON.parse(userdetail);
        let thisLocal = this;
        let isTrue = true;
        let status = 0;
    ...
    async StatusRobot(AuthenticationURL, sessionid, RoboID, JobKey) 
    {
        let userdetail = await SessionHelper.Get(sessionid);
        userdetail = JSON.parse(userdetail);
        let thisLocal = this;
    ...
    async GetJobLogs(AuthenticationURL, sessionid, jobId, Robotname)
    {
        let userdetail = await SessionHelper.Get(sessionid);
        userdetail = JSON.parse(userdetail);
        let thisLocal = this;
